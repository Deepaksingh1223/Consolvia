        ...
        <ol className="mt-14 hidden lg:grid lg:grid-cols-5 lg:gap-6">
          {PROCESS_STEPS.map((step, index) => (
            <Reveal as="li" key={step.number} delay={index * 90} className="group relative">
              ...
              <h3 className="mt-2 text-base font-bold leading-snug tracking-tight text-ink">
                {step.title}
              </h3>
          ...
          {PROCESS_STEPS.map((step, index) => (
            <li key={step.number} className="relative flex gap-5">
              <div className="flex flex-col items-center">
                <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-line bg-white text-brand">
